    ...
    @checkUpcomingEvents = () ->
      ...
      for event in self.upcomingEvents
        ...
        if (time_until_event > 0) and (time_until_event < (self.config.upcoming_check_headsup))
          self.robot.messageRoom '!FsFLbKGMcUXEMBxZdu:tomesh.net', "Reminder: *" + event.title + "* at *" + friendly_time + "*. This is taking place at *" + event.location + "*. More details here: " + event.url
